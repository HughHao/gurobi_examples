# -*- coding: utf-8 -*-
# @Time : 2022/12/11 13:54
# @Author : hhq
# @File : procution_planning2.py
# -*- coding: utf-8 -*-
"""
...
for order in ORDERS:
    for step in STEPS:
        ...
        model.addConstrs(
            sum(useResource[resource, slot, order, step] for slot in SLOTS) <= STEP_RESOURCES[order, step, resource] for
...
for order in ORDERS:
    for step in STEPS:
        ...
        model.addConstrs(
            sum(useMould[mould, slot, order, step] for slot in SLOTS) <= STEP_MOULDS[order, step, mould] for mould in
            MOULDS)

# 4. 每个设备/模具每个时段中只能分配被一个产品的一道工序占用
model.addConstrs(
    sum(useResource[resource, slot, order, step] for order in ORDERS for step in STEPS) <= 1 for resource in RESOURCES
    for slot in SLOTS)
model.addConstrs(
    sum(useMould[mould, slot, order, step] for order in ORDERS for step in STEPS) <= 1 for mould in MOULDS for slot in
    SLOTS)
...
for resource in RESOURCES:
    for slot in SLOTS:
        for order in ORDERS:
            for step in STEPS:
                model.addConstr(startR[resource, slot, order, step] <= start[order, step] + (
                            1 - useResource[resource, slot, order, step]) * M)
                model.addConstr(startR[resource, slot, order, step] >= start[order, step] - (
                            1 - useResource[resource, slot, order, step]) * M)
                model.addConstr(startR[resource, slot, order, step] <= useResource[resource, slot, order, step] * M)
                model.addConstr(endR[resource, slot, order, step] <= start[order, step] + STEP_TIME[order, step] + (
                            1 - useResource[resource, slot, order, step]) * M)
                model.addConstr(endR[resource, slot, order, step] >= start[order, step] + STEP_TIME[order, step] - (
                            1 - useResource[resource, slot, order, step]) * M)
...
for resource in RESOURCES:
    for slot in SLOTS:
        model.addConstr(rSlotStartTime[resource, slot] == sum(
            startR[resource, slot, order, step] for order in ORDERS for step in STEPS))
        model.addConstr(rSlotEndTime[resource, slot] == sum(
            endR[resource, slot, order, step] for order in ORDERS for step in STEPS))
...
for mould in MOULDS:
    for slot in SLOTS:
        for order in ORDERS:
            for step in STEPS:
                model.addConstr(startM[mould, slot, order, step] <= start[order, step] + (
                            1 - useMould[mould, slot, order, step]) * M)
                model.addConstr(startM[mould, slot, order, step] >= start[order, step] - (
                            1 - useMould[mould, slot, order, step]) * M)
                model.addConstr(startM[mould, slot, order, step] <= useMould[mould, slot, order, step] * M)
                model.addConstr(endM[mould, slot, order, step] <= start[order, step] + STEP_TIME[order, step] + (
                            1 - useMould[mould, slot, order, step]) * M)
                model.addConstr(endM[mould, slot, order, step] >= start[order, step] + STEP_TIME[order, step] - (
                            1 - useMould[mould, slot, order, step]) * M)
...
for mould in MOULDS:
    for slot in SLOTS:
        model.addConstr(
            mSlotStartTime[mould, slot] == sum(startM[mould, slot, order, step] for order in ORDERS for step in STEPS))
        model.addConstr(
            mSlotEndTime[mould, slot] == sum(endM[mould, slot, order, step] for order in ORDERS for step in STEPS))
...
for resource in RESOURCES:
    for slot in SLOTS:
        ...
        if rSlotEndTime[resource, slot].x > rSlotStartTime[resource, slot].x:
            string = resource + '%10s' % str(rSlotStartTime[resource, slot].x) + '%10s' % str(
                rSlotEndTime[resource, slot].x) + '\t'
            stop = 0
            for order in ORDERS:
                for step in STEPS:
                    if abs(useResource[resource, slot, order, step].x - 1) <= 0.01:
