    ...
    for j in range(1, Batches[1]+1):
        if z[j-1].x != 0:
            for i in Patterns.keys():
                ...
                if value != 0:
                    for ii in range(1, round(value) + 1):
                         Solutions[Patterns[i][ii]] = batchnum
                    del Patterns[i][1:round(value) + 1]
            batchnum += 1
...
def OutputResult(Solutions, Record):
    ...
    workbook.save('Result_按仓库(V2).xlsx')
...
try:
    ...
    BuildFirstStageModel(Packages, Warehouse, Patterns, Batches, PackageLimit, GoodsLimit, BatchePattern, SolveTime)
